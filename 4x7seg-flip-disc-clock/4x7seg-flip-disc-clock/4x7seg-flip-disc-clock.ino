/*----------------------------------------------------------------------------------*
   A simple example of controlling 4 x 7-segment and one 3x1 flip-disc displays
...
void setup()
{
  ...
  /* The function is used to turn on all displays */
  //Flip.All();
...
void loop()
{
  ...
  if (now.Hour() != last.Hour() || now.Minute() != last.Minute()) {
  ...
  } else if (last.Second() != now.Second()) {
    // Flip one disc to indicate seconds
    ...
    /* 0  1  2  3  4
      ...
      15           9
      14 13 12 11 10 */
    Flip.Disc_7Seg(1, 14, now.Second()%2); // last argument can be 0 to turn off
  }
  last = now;
  delay(200);
